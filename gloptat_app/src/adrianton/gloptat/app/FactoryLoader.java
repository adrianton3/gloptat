...
public class FactoryLoader {
	...
	private static <T extends OAFactory> Iterator<T> initServiceLoader(ClassLoader cl, Class<T> clazz) {
		return ServiceLoader.load(clazz, cl).iterator();
	}

	public static <T extends OAFactory> ArrayList<T> getFactories(String dir, Class<T> clazz) {
		...
		if(!iterator.hasNext())
			Dbo.out("No optimization algorithms could be found in factories/"
					+ "\nUsing only internal optimization algorithms");
		
		instance.add((T)new RandomSearcherFactory());
